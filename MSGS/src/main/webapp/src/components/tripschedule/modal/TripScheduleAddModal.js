...
const TripScheduleAddModal = ({
	...
	const feedbackHandler = () => { //장소 7개 초과하여 담으려는 경우에 호출됨
	...
	const placeCheckHandler = (isChecked, data) => {
		...
		if (isChecked) {
			...
			setCheckedPlaces(checkedPlaces.filter((obj) => obj.contentid !== data.contentid)) // 선택 해제
		}
	}
	...
	const confirmSelection = () => {
		...
		let updatedObj = { ...prevObj }
			...
			if (updatedObj[ orderDay ]) {
				...
				return { ...item, order: index + 1, placeOrder: item.placeOrder ? maxPlaceOrder : null }
			})
		...
		<div className={styleModal['modal-container']}>
			<div className={styleModal['modal-wrapper']}>
				<div className={styleModal['modal-top']}>
					...
					<img className={styleModal['icon-close']} src={process.env.PUBLIC_URL + '/images/icon_close.png'} alt='icon_close' onClick={closeModal} />
				</div>
				...
				<button className={styleModal['select-complete-btn']} onClick={confirmSelection}>
					선택 완료
				</button>
			</div>
