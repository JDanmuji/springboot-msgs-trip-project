...
import styles from './StarRatingModal.module.css';

// 별점 별모양 클릭해서 매기는 컴포넌트입니다.
// 별 모양 클릭한 만큼 점수가 숫자로 보여지게 하는 함수입니다. 
const StarRatingModal = ({rating, handleRatingChange}) => {
    const handleInputChange = (event) => {
        handleRatingChange(event);
        document.getElementById('total-score-modal').textContent = event.target.value;
      };

    return (
        <>
            <p className={styles["star-score-modal"]}>Total score for this trip: <span id="total-score-modal">0</span></p> {/* 별점 점수 */}
            <div className={`${styles["star-rating-modal"]} space-x-4 mx-auto`}>
