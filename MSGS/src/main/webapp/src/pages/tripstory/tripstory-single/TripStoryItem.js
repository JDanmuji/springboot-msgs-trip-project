...
import style2 from "./TripStoryItem.module.css";
...
const TripStoryItem = (props) => {
  ...
  return (
    <div className={style2["trip-story-item-div"]}>
      <TripStoryItemImg tripImg={props.tripImg} length={length} />
      <div className={style2["trip-story-item-info"]}>
        <p>
          {props.userImg.length > 0 ? (
            <img
              className={style2["trip-story-item-user-profile"]}
              src={props.userImg}
              alt="userImg"
            />
          ) : (
        ...
        <div className={style2["trip-story-item-info-inner1"]}>
          <p>{props.userNickname}</p>
          <p>{getDuration()}</p>
        </div>
        <p className={style2["trip-story-item-title"]}>{props.tripTitle}</p>
        <p className={style2["trip-story-item-tag"]}>{props.tripTag}</p>
      </div>
    </div>
  );
};
