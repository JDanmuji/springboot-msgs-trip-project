  ...
  return (
    <div className={styles['width-wrapper']}>
      ...
      <div className={styles['nickname-area']}>
        <input type='text' className={styles['nickname-input']} placeholder='설정할 닉네임 입력' 
               value={nickname}
               onChange={(e) => setNickname(e.target.value)}
               />
               <button className={styles['clear-btn']} onClick={handleClearInput}>
                  <IoMdCloseCircle className={styles['clear-icon']}/>
               </button>
      </div>
      <div className={styles['ok-btn-area']}>
        <div className={styles['nickname-ok-btn']} >
              <button disabled={!isNicknameValid}>확인</button>
        </div>
      </div>
